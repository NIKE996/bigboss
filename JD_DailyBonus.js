...
[Script]
京东多合一签到 = type=cron,cronexp=5 0 * * *,wake-system=1,timeout=60,script-path=https://raw.githubusercontent.com/NIKE996/bigboss/main/JD_DailyBonus.js

获取京东Cookie = type=http-request,pattern=https:\/\/api\.m\.jd\.com\/client\.action.*functionId=signBean,script-path=https://raw.githubusercontent.com/NIKE996/bigboss/main/JD_DailyBonus.js
...
[Script]
cron "5 0 * * *" tag=京东多合一签到, script-path=https://raw.githubusercontent.com/NIKE996/bigboss/main/JD_DailyBonus.js

http-request https:\/\/api\.m\.jd\.com\/client\.action.*functionId=signBean tag=获取京东Cookie, script-path=https://raw.githubusercontent.com/NIKE996/bigboss/main/JD_DailyBonus.js
...
5 0 * * * https://raw.githubusercontent.com/NIKE996/bigboss/main/JD_DailyBonus.js, tag=京东多合一签到, img-url=https://raw.githubusercontent.com/NobyDa/mini/master/Color/jd.png,enabled=true

[rewrite_local]
# 获取京东Cookie. 
# 注意此为远程路径, 低版本用户请自行调整为本地路径.
https:\/\/api\.m\.jd\.com\/client\.action.*functionId=signBean url script-request-header https://raw.githubusercontent.com/NIKE996/bigboss/main/JD_DailyBonus.js
...
function ReadCookie() {
  ...
  const EnvInfo3 = $nobyda.isJSBox ? "JD_Cookie3" : "CookiesJD3"
  const EnvInfo4 = $nobyda.isJSBox ? "JD_Cookie4" : "CookiesJD4"
  const EnvInfo5 = $nobyda.isJSBox ? "JD_Cookies" : "CookiesJD"
  if (DeleteCookie) {
    if ($nobyda.read(EnvInfo) || $nobyda.read(EnvInfo2) || ($nobyda.read(EnvInfo3) || '[]') != '[]') {
      ...
      $nobyda.write("", EnvInfo4)
      $nobyda.write("", EnvInfo5)
  ...
  if (KEY) {
    ...
    DualKey = DualKey || $nobyda.read(EnvInfo3)
    DualKey = DualKey || $nobyda.read(EnvInfo4)
    OtherKey = OtherKey || $nobyda.read(EnvInfo5)
    if ($nobyda.isJSBox || $nobyda.isNode) {
      if (Key) $nobyda.write(Key, EnvInfo);
      if (DualKey) $nobyda.write(DualKey, EnvInfo2);
      if (DualKey) $nobyda.write(DualKey, EnvInfo3);
      if (DualKey) $nobyda.write(DualKey, EnvInfo4);
      if (OtherKey) $nobyda.write(OtherKey, EnvInfo5);
